...
데이터 영역 길이가 10이고 실제 데이터의 길이가 10보다 작을 때 작은 길이만큼의 영역에 해당 문자 Char로 채우는 기능을 한다. 예로 데이터 영역이 10이고 데이터의 길이가 7일 때 spaceChar "&#42;"로 설정되었다면 전송하는 데이터는 “1234567***” 형태로 전송합니다.<br/>
