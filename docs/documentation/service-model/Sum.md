...
<!--SUM-->
<b style="font-size: 20px">1) SUM</b><br/>
입력 Dataset Object에 대하여 구하는 Component입니다.

<b style="font-size: 20px">(1) BOC 이미지</b><br/>
BOC 컴포넌트 목록에서 SUM BOC을 선택하여 Drag-Drop으로 화면작업 영역에 디자인합니다. <br/>
<img src="../../.vuepress\public\documentation\service-model\BOC\ChangeBOC\SUMBoc.png" style="position: relative;top: 5px; width:150px;"> <br/>
<p style="background-color: rgb(216, 190, 228);">데이터베이스의 데이터의 결과집합 중 출력할 BOC에서 사용할 대상 컬럼 부를 표시합니다.</p>
...
Default는 전체 항목이 되며 입력OBJECT의 결과집합 중 이후 BOC에서 사용할 대상 컬럼을 지정합니다. <br/>
지정된 Column이외 자동 생성되는 Column으로 PARTCNT 항목이 추가 된다. PARTCNT 항목의 값은 소계, 총계의 값은 SUM 처리된 Record의 Count가 표시된다. 원본 True 상태에서 원본 Record에는 ‘0’ 값이 setting 됩니다. <br/>
 PARTCNT를 이용하여 해당 record의 속성 확인이 가능합니다.

- <b style="font-size: 20px">합계 기준 </b><br/>
입력되는 Object의 적용 컬럼을 정의합니다.

- <b style="font-size: 20px">합계 대상 </b><br/>
합계를 구하고자 하는 컬럼을 등록됩니다.

- <b style="font-size: 20px">합계 출력 유형 </b><br/>
화면에 출력되는 데이터 형태를 True/False로 설정합니다. Default 지정은 소계, 총계는 True이며 원본은 false로 지정됩니다.

<b style="font-size: 20px">➂ 출력부</b><br/>
- <b style="font-size: 20px">Record-Count/Page </b><br/>
Pagination 시 페이지당 레코드 건수를 설정됩니다.
...
- <b style="font-size: 20px">SUM 반올림 유형 </b><br/>
합계 출력하는 데이터에 ROUND/ROUNDDOWN을 지정합니다.

- <b style="font-size: 20px">출력 포맷 </b><br/>
합계 출력하는 데이터에 포맷을 입력한다. 포맷방식은 Java에서 사용하는 DecimalFormat가 동일한 방법입니다.
